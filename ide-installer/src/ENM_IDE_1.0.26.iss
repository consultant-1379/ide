...
#define MyIDEvhdReizer_File "VhdResizer.msi"
...
#define MyCloneENMIDEFileBatch "CloneENM-IDE.bat"
#define MySubstCloneENMIDEFileBatch "substSignumInCloneScript.bat"
...
#define MyBashAliasesFileName "ATT69520.bash_aliases"
#define MyCrontabDefaultFileName "crontab.cron"
#define MyCloneENMIDEScriptFileName "CloneENM-IDE.sh"
...
[Files]
Source: "{#MyBaseDir}\{#MySourceDir}\{#MyScriptsDir}\*"; DestDir: "{code:NewTargetDir}\{#MyScriptsDir}"; Flags: ignoreversion; AfterInstall: SetScriptNames('{code:NewTargetDir}\{#MyScriptsDir}')
; Source: "{#MyBaseDir}\{#MySourceDir}\{#MyScriptsDir}\{#MyCrontabFile}"; DestDir: "{code:NewTargetDir}\{#MyScriptsDir}\{code:GetUser}.cron"; Flags: ignoreversion
Source: "{#MyBaseDir}\{#MySourceDir}\{#MyCertificateDir}\{#MyIDEInstallerCertificate}"; DestDir: "{code:NewTargetDir}\{#MyCertificateDir}"; Flags: ignoreversion
...
Source: "{#MyIDEVmDevIDE_Dir}\*"; DestDir: "{code:NewTargetUserDir}\{#MyRadixIDE_Dir}"; Flags: ignoreversion recursesubdirs createallsubdirs

Source: "{#MyBaseDir}\{#MySourceDir}\{#MyBashAliasesFileName}"; DestDir: "{code:NewTargetDir}"; CopyMode: alwaysoverwrite; Flags: ignoreversion
...
//TODO
function GetInstallPathOfGitClient(const strSource: String; const strToBeRemoved: String):String;
var
  strAlphabet, strTmpFileName: String;
  i, j, k, iLen, ResultCode:Integer;
...
begin
   ...
   Exec('cmd.exe', '/C git --info-path > ' + strTmpFileName, '', SW_HIDE, ewWaitUntilTerminated, ResultCode);
   //Exec('cmd.exe', '/C git --info-path', '', SW_HIDE, ewWaitUntilTerminated, ResultCode);
...
begin
  ...
  iPos :=  Pos('Git',strToFind);
  if iPos <> 0 then
    begin
       //Get the start path of the already installed product: the C:\Program...XXX\
...
//InnoSetup default Event virt         ual Function that is runned before install files on target, just to allow prerequisites verify
...
begin
  ...
  fInstallGit := false;
  strGitIntRadixPath := ExpandConstant('C:\Program Files (x86)\');
  //strGitIntRadixPath := ExpandConstant(GetGitExePath());
  //GIAG
...
begin
  ...
  PersonalPage := CreateInputQueryPage(wpWelcome,
                                       ...
                                       'Here you can see what Ide-installer have found as your username signum in your Windows PC',
  ...
  PersonalPageEditIndex := PersonalPage.Add('Ide-Installer found that your Windows Environment variable USERNAME is:', False);
  ...
  SelectDestLocPage := CreateInputDirPage(PersonalPage.ID,
                                          'Select Personal Data Location',
                                          'Where should personal data files be stored?',
                                          'Personal data files must be stored under a new path with root <your signum>\ENM_IDE in the folder below. To continue, click Next.',
                                          False,
                                          'New Folder');
  ...
  SelectDestLocPage.Edits[FirstEditIndex].Visible := True;
  ...
  SelectDestLocPage.Edits[SecondEditIndex].Visible := False;
...
begin
  ...
  //else 
  //  if Param = 'GitSignum' then
  //    Result := PersonalPage.Values[1]
  else
    Result := 'Empty';
...
var
  strSSHPath, strSSHFile, strTmp, strLog, strSSHConfigName: String;
  strGitUserSignum, strGitCmd, strGitBashCmd, strGitPath: String;
  rslt: Boolean;
  ResultCode: Integer;
  MyFile : TStrings;
  MyText : String;
begin
     ...
     //Must to understand where "git.exe" has been just now installed
     //strGitPath := GetGitExePath();
     //strGitIntRadixPath := GetGitExePath();
     strGitBashCmd := ExpandConstant(strGitIntRadixPath+'\Git\bin\sh.exe');
     strGitCmd := ExpandConstant(strGitIntRadixPath+'\Git\bin\git.exe');
     
     if not DirExists(strSSHPath) then
       begin
         //Exec('cmd.exe', '/C cd & pause', '', SW_SHOW, ewWaitUntilTerminated, ResultCode);         
         //Exec('cmd.exe', '/C whoami & pause', '', SW_SHOW, ewWaitUntilTerminated, ResultCode);         
         //Exec('cmd.exe', '/C echo whoami | "C:\Program Files (x86)\Git\bin\sh.exe" --login -i', '', SW_SHOW, ewWaitUntilTerminated, ResultCode);         
         //Exec('cmd.exe', '/C echo ssh-keygen -t rsa | "C:\Program Files (x86)\Git\bin\sh.exe" --login -i', '', SW_SHOW, ewWaitUntilTerminated, ResultCode);
         strGitUserSignum := GetUser('Signum');
         
         //Set the gerrit signum username to generate ssh key using ssk-keygen in future: git config --global user.name "Your Name"
         Exec('cmd.exe', '/C '+strGitCmd+' config --global user.name '+strGitUserSignum, '', SW_SHOW, ewWaitUntilTerminated, ResultCode);
         Exec('cmd.exe', '/C echo | echo ssh-keygen -t rsa -q -N "" | "'+strGitBashCmd+'" --login -i', '', SW_SHOW, ewWaitUntilTerminated, ResultCode);

         //I has been used current USERNAME and default path USERNAME\.ssh to create ssh key.
         //Now we have to overwrite both keys with parameters that are using the signum/Username definied in Gerrit for this USERNAME. 
         //using ssh-keygen command with desired parameters: ssh-keygen -t rsa -f /c/Users/xgiogiu/.ssh/id_rsa -C "xgiogiu@BGE377280"
         //Exec('cmd.exe', '/C echo | echo ssh-keygen -t rsa -q -N "" | "C:\Program Files (x86)\Git\bin\sh.exe" --login -i', '', SW_SHOW, ewWaitUntilTerminated, ResultCode);
       ...
       if not FileOrDirExists(strSSHFile) then
         begin
           ...
           Exec('cmd.exe', '/C echo | echo ssh-keygen -t rsa | "'+strGitBashCmd+'" --login -i', '', SW_SHOW, ewWaitUntilTerminated, ResultCode);
           Exec('cmd.exe', '/C copy '+strSSHConfigName+' '+strSSHPath, '', SW_HIDE, ewWaitUntilTerminated, ResultCode);
         end;
...
function IsGitAlreadyInstalled(): Boolean;
var
  strBuff,strPath, strLog, strTmp2, strLogPath: String;
  FreeMB, TotalMB: Cardinal;
  ResultCode: Integer;
  fRslt: Boolean;
begin
...
begin
  ...
  begin
     ...
     //Must to understand where "git.exe" has been just now installed
     //strPath := ExpandConstant(GetGitExePath());
...
//Replace a string into a file and save that file.
...
//Global procedure that is able to align the name of some script (i.e. the " crontab.cron") using the target user's signum  GIAN
procedure SetScriptNames(const Dir: String);
var
  ResultCode: Integer;  
  strFullNameFile, strUserSignum, strFullTargetDirFile: String;
begin
  strFullNameFile := ExpandConstant(Dir+'\{#MyCrontabDefaultFileName}');
  
  strUserSignum := GetUser('Signum');
  strFullTargetDirFile := ExpandConstant(Dir+'\'+strUserSignum+'.cron');
  //getUser GGG

  Exec('cmd.exe', '/C move '+strFullNameFile+' '+strFullTargetDirFile, '', SW_HIDE, ewWaitUntilTerminated, ResultCode);
end;
...
var
  ResultCode, iPos: Integer;  
  S: String;
  strFullNameFileToFind, strGitFileName, strGitUserSignum, strUserNameMail, strTmpFileName: String;
  strWorkingDir, strGitBashPath, strOrigPath, strNewPath, strToFind, strGitCmd: String;
  Rslt: boolean;
...
begin
  ...
  //Must to understand where "git.exe" has been just now installed
  strGitIntRadixPath := GetGitExePath();
  strGitBashPath := ExpandConstant(strGitIntRadixPath+'\Git\bin');
...
//    end;
  ...
  //  begin
      RegQueryStringValue(HKEY_LOCAL_MACHINE,
                          'SYSTEM\CurrentControlSet\Control\Session Manager\Environment',
                          'Path', strOrigPath);
      strNewPath := ExpandConstant(strOrigPath+';'+strGitBashPath);
  ...
  //TODO
  //get the signum on what the user is registered as "Username" in remote Gerrit repo area db. 
  strGitUserSignum := GetUser('Signum');
  strGitCmd := ExpandConstant(strGitBashPath+'\git.exe');
  //GIAG
  //Set the gerrit signum username to generate ssh key using ssk-keygen in future: git config --global user.name "Your Name"
  Exec('cmd.exe', '/C '+strGitCmd+' config --global user.name '+strGitUserSignum, '', SW_SHOW, ewWaitUntilTerminated, ResultCode);
end;
...
var
  ...
  ResultCode,I,iSize, ReturnCode:   Integer;
  Rslt: boolean;
  strDestName, strDestPath: String;
  strTmp1, strTmp2, strGetVagranfileBatchName, strCloneENMIDEBatchName: String;
  strGitBashCmd, strGitCmd, strSubstSignumInCloneENMIDEBatchName: String;
begin
  Rslt := false;
  ...
  if StringChangeEx(strGitreposDir, '\', '/', True) > 0 then //Only get Vagrantfile if text has been changed. Otherwise it won't work..
    begin
      ...
      //non e' detto che Git venga installato sotto la Program Files (x86). Bisogna usare la funzione vista sopra con wmic per il path corretto.
      strGitBashCmd := ExpandConstant(strGitIntRadixPath+'\Git\bin\sh.exe');
      Exec('cmd.exe', '/C echo "'+strGitBashCmd+'" --login -i -c "cd '+strGitreposDir+' && echo -ne ''\ny'' | git archive --remote=ssh://'+strUserSignum+'@gerrit.ericsson.se:29418/OSS/com.ericsson.de/ide.git HEAD:vagrant Vagrantfile | tar -x" > '+strGetVagranfileBatchName, '', SW_HIDE, ewWaitUntilTerminated, ResultCode);
      Exec('cmd.exe','/C '+strGetVagranfileBatchName,'', SW_HIDE, ewNoWait, ResultCode);
      strDestPath := ExpandConstant('{code:NewTargetDir}');

      //**************************************************************
      //***************   IDE_Clone Dos Batch script ************
      //**************************************************************  
      strCloneENMIDEBatchName := ExpandConstant('{code:NewTargetDir}\{#MyCloneENMIDEFileBatch}');
      StringChangeEx(strDestPath, '\', '/', True);
      Exec('cmd.exe', '/C echo "'+strGitBashCmd+'" --login -i -c "cp '+strDestPath+'/scripts/{#MyCloneENMIDEScriptFileName} '+strGitReposDir+' && cd '+strGitReposDir+' && ./{#MyCloneENMIDEScriptFileName}" > '+strCloneENMIDEBatchName, '', SW_HIDE, ewWaitUntilTerminated, ResultCode);
      
      //***************** Correct the content of the script CloneENM-IDE.sh to substitute "yoursignum" string with the real user's signum **********
      strSubstSignumInCloneENMIDEBatchName := ExpandConstant('{code:NewTargetDir}\{#MySubstCloneENMIDEFileBatch}');
      Exec('cmd.exe', '/C echo "'+strGitBashCmd+'" --login -i -c "sed -i s/yoursignum/'+strUserSignum+'/g '+strDestPath+'/scripts/{#MyCloneENMIDEScriptFileName}" > '+strSubstSignumInCloneENMIDEBatchName, '', SW_HIDE, ewWaitUntilTerminated, ResultCode);      
      Exec('cmd.exe', '/C '+strSubstSignumInCloneENMIDEBatchName, '', SW_HIDE, ewWaitUntilTerminated, ResultCode);      
      //***************************************************************
